#!/usr/bin/env python3
"""
Together.ai experiment runner for safe-guideline-tooling-eval.
Runs all 3 models in parallel on 500 cases.
"""
...
def run_together_experiment(
    model_name: str,
    cases: list,
    output_dir: Path,
    seed: int = 42,
):
    """Run experiment with Together.ai model."""
    ...
    with Progress(
        ...
        task = progress.add_task(f"[{model_name}] Processing...", total=len(cases))
        
        for case in cases:
            case_id = case["case_id"]
            progress.update(task, description=f"[{model_name}] {case_id}")
...
"""
    ...
    console.print(f"\n[bold green]Complete: {model_name}[/bold green]")
    console.print(f"Total: {len(results)} | Valid: {len(valid_results)}")
...
def main():
    ...
    output_dir = Path("results") / f"run_together_{model_safe}_{timestamp}"
    
    console.print(f"\n[bold]Together.ai Experiment[/bold]")
    console.print(f"Model: {args.model}")
    console.print(f"Cases: {len(cases)}")
    console.print(f"Output: {output_dir}")
    
    run_together_experiment(
